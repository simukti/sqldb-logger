...
func TestStatement_Close(t *testing.T) {
	...
	t.Run("Error", func(t *testing.T) {
		...
		stmt := &statement{query: q, Stmt: stmtMock, logger: to.testLogger, id: to.testLogger.opt.uidGenerator.UniqueID(), connID: to.testLogger.opt.uidGenerator.UniqueID()}
		err := stmt.Close()
		assert.Error(t, err)
...
func TestStatement_QueryContext2(t *testing.T) {
	...
	err = json.Unmarshal(to.bufLogger.Bytes(), &connOutput)
	fmt.Println(to.bufLogger.String())
	assert.NoError(t, err)
	...
	err = json.Unmarshal(to.bufLogger.Bytes(), &stmtOutput)
	assert.NoError(t, err)
...
func TestStatement_CheckNamedValue(t *testing.T) {
	...
	t.Run("Error", func(t *testing.T) {
		...
		stmt := &statement{Stmt: stmtMock, logger: to.testLogger, id: to.testLogger.opt.uidGenerator.UniqueID(), connID: to.testLogger.opt.uidGenerator.UniqueID()}
