...
//		custLogger := *ml.testLogger
//		ml.testOpt = &custOpt
//
//		stmt := &statement{query: q, Stmt: stmtMock, logger: &custLogger, id: ml.testOpts.uidGenerator.UniqueID(), connID: ml.testOpts.uidGenerator.UniqueID()}
//		_, err := stmt.Exec([]driver.Value{"testid"})
//		assert.NoError(t, err)
//
//		var output bufLog
...
//	fmt.Println(ml.bufLogger.String())
//	assert.NoError(t, err)
//	assert.Equal(t, LevelDebug.String(), connOutput.Level)
//	assert.Equal(t, conn.id, connOutput.Data[ml.testOpts.connIDFieldname])
//
...
//	assert.Equal(t, conn.id, stmtOutput.Data[ml.testOpts.connIDFieldname])
